...
// GLIMPSE_DELAY lets the caption finish animating in first; GLIMPSE_DURATION
// must match the glimpse keyframes in styles.css, because the class is removed
// when it elapses so that :hover can take back control afterwards.
const GLIMPSE_DELAY = 750;
const GLIMPSE_DURATION = 2000;
...
/**
 * The same bilingual reveal the rest of the invitation uses: English by
 * default, Tamil on hover or focus. In Tamil mode it simply renders Tamil.
 * `pair` is the [english, tamil] tuple from journey.js.
 */
function Line({pair,lang,as:Tag='span',className=''}){
 if(lang)return <Tag className={`txt tamil ${className}`} lang="ta">{pair[1]}</Tag>;
...
export default function TempleJourney({language,reducedMotion}){
 ...
 useEffect(()=>{
  // Nothing to reveal when the whole page is already Tamil, and a paused or
  // offscreen journey should not be flickering to itself.
  if(lang||fallback||!active){setGlimpse(false);return;}
  setGlimpse(false);
  const show=setTimeout(()=>setGlimpse(true),GLIMPSE_DELAY);
